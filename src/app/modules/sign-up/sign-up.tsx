...
import {
  FormDataModel,
  FormDataValidModel,
} from "../../core/models/FormData.models";
import {
  confirmPassword,
  emailValid,
  singUpError,
...
const SignUP: React.FC = () => {
  ...
  const { isSuccess, isLoading, isError, date } = useAppSelector(
    (state) => state[KEY_SIGN_UP]
  );
  ...
  const [errorMessages, setErrorMessages] =
    useState<string>(requirementsMassages);
  ...
  useEffect(() => {
    if (!!formData.email) {
      ...
      cloneValid.email = value
        ? (cloneValid.email = false)
        : (cloneValid.email = true);
      setNotValid(cloneValid);
    }
  }, [formData.email]);
  ...
  return (
    <>
      ...
      >
        ...
        >
          {notValid.confPassWord && (
            <span className="requirements">{errorMessages}</span>
          )}
        </InputPassWord>
